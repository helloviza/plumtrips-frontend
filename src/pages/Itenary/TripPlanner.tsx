...
export default function TripPlanner() {
  ...
  if (!tripData || !tripData.itinerary) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px", background: "linear-gradient(135deg, #f8fbff 0%, #eef4ff 100%)" }}>
        <div style={{ textAlign: "center", maxWidth: 560, width: "100%", background: "#fff", borderRadius: 28, padding: "36px 24px", boxShadow: "0 20px 60px rgba(15,23,42,0.08)" }}>
          ...
          <h1 style={{ fontSize: "clamp(1.5rem, 5vw, 2rem)", marginBottom: 12, color: C.navy }}>No itinerary found</h1>
  ...
  return (
    <div style={{ fontFamily: "'Poppins', sans-serif", background: "linear-gradient(180deg, #f8fbff 0%, #f2f6fb 100%)", minHeight: "100vh", paddingBottom: 100 }}>
      <style>{`
        ...
        .trip-planner-shell { color: #0f172a; overflow-x: hidden; }
        .glass-card {
        ...
        /* ── Layout primitives pulled out of inline styles so media
           queries below can actually override them (inline styles beat
           plain CSS, so the base values live here instead). ── */
        .trip-planner-hero-inner {
          max-width: 1200px;
          margin: 0 auto;
          padding: 0 40px;
        }
        ...
        .hero-bg-image {
          ...
          bottom: 0;
          width: 42%;
          background-image: url('/__mockup/images/bali-hero.png');
          background-size: cover;
          background-position: center;
          opacity: 0.24;
          -webkit-mask-image: linear-gradient(to right, transparent, black 42%);
        ...
        /* ── Tablet (≤ 1024px): main content stacks, hero/main padding
           tightens a notch, metrics move to 2-up. ── */
        @media (max-width: 1024px) {
          .trip-planner-main { grid-template-columns: 1fr !important; }
          .trip-planner-hero-inner { padding: 0 32px !important; }
          .trip-planner-main { padding: 0 32px !important; }
          .trip-planner-metrics { grid-template-columns: 1fr 1fr !important; }
        }

        /* ── Small tablet / large phone (≤ 720px) ── */
        @media (max-width: 720px) {
          .trip-planner-hero { padding: 40px 0 80px !important; }
          .trip-planner-hero-inner { padding: 0 20px !important; }
          .trip-planner-main { padding: 0 20px !important; margin-top: -56px !important; gap: 18px !important; }
          .flight-hotel-row { grid-template-columns: 1fr !important; }
          .hero-bg-image { width: 55% !important; opacity: 0.16 !important; }
          .day-card-header { gap: 10px !important; }
          .glass-card-pad { padding: 20px !important; }
        }

        /* ── Phones (≤ 480px) ── */
        @media (max-width: 480px) {
          .trip-planner-metrics { grid-template-columns: 1fr 1fr !important; gap: 10px !important; }
          .metric-card { padding: 12px 14px !important; }
          .soft-pill { font-size: 10.5px !important; padding: 6px 10px !important; }
          .summary-chip { font-size: 11px !important; padding: 7px 10px !important; }
          .hero-blob { display: none !important; }
          .hero-bg-image { display: none !important; }
          .quick-edit-chip { flex: 1 1 auto; justify-content: center; }
        }
      `}</style>
      ...
      <div className="trip-planner-shell">
        ...
        >
          ...
          <div className="trip-planner-hero-inner" style={{ position: "relative", zIndex: 10 }}>
            <div className="soft-pill" style={{ marginBottom: 18 }}>
              <Sparkles size={13} />
              AI-Powered Journey
            </div>

            <h1 style={{ fontSize: "clamp(1.7rem, 6vw, 3.2rem)", fontWeight: 800, marginBottom: 16, maxWidth: 820, lineHeight: 1.1 }}>
              {tripTitle}
            </h1>
            <p style={{ fontSize: "clamp(0.9rem, 2.4vw, 1.05rem)", color: "rgba(255,255,255,0.84)", maxWidth: 700, marginBottom: 24, lineHeight: 1.7 }}>
              {summary}
            </p>
            ...
            <div className="trip-planner-metrics">
              <div className="metric-card">
                <div style={{ fontSize: 12, letterSpacing: "0.12em", color: "rgba(255,255,255,0.68)", marginBottom: 6 }}>DATES</div>
                <div style={{ fontSize: "clamp(13px, 3vw, 16px)", fontWeight: 700, wordBreak: "break-word" }}>{slots.departDate || "TBD"} – {slots.returnDate || "TBD"}</div>
              </div>
              <div className="metric-card">
                <div style={{ fontSize: 12, letterSpacing: "0.12em", color: "rgba(255,255,255,0.68)", marginBottom: 6 }}>TRAVELERS</div>
                <div style={{ fontSize: "clamp(13px, 3vw, 16px)", fontWeight: 700 }}>{slots.adults || 1} Adult{slots.children ? ` + ${slots.children} Child` : ""}</div>
              </div>
              <div className="metric-card">
                <div style={{ fontSize: 12, letterSpacing: "0.12em", color: "rgba(255,255,255,0.68)", marginBottom: 6 }}>TRIP LENGTH</div>
                <div style={{ fontSize: "clamp(13px, 3vw, 16px)", fontWeight: 700 }}>{tripLength} Night{tripLength === 1 ? "" : "s"}</div>
              </div>
              <div className="metric-card">
                <div style={{ fontSize: 12, letterSpacing: "0.12em", color: "rgba(255,255,255,0.68)", marginBottom: 6 }}>TRIP VIBE</div>
                <div style={{ fontSize: "clamp(13px, 3vw, 16px)", fontWeight: 700 }}>{slots.tripVibe || "Personalized"}</div>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="trip-planner-main">
          <div style={{ display: "flex", flexDirection: "column", gap: 24, minWidth: 0 }}>
            <section className="glass-card glass-card-pad" style={{ padding: 28 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
                <div>
                  ...
                  <h2 style={{ fontSize: "clamp(1.35rem, 4vw, 1.8rem)", fontWeight: 800, color: C.navy, marginBottom: 8 }}>{tripTitle}</h2>
            ...
            {outboundFlight || returnFlight || hotel ? (
              <section style={{ display: "grid", gap: 18 }}>
                <div className="flight-hotel-row">
                  {outboundFlight ? (
                    <div className="glass-card glass-card-pad" style={{ padding: 24, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(255,104,44,0.12)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                  ...
                  {returnFlight ? (
                    <div className="glass-card glass-card-pad" style={{ padding: 24, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(45,140,255,0.12)", display: "grid", placeItems: "center", flexShrink: 0 }}>
                ...
                {hotel ? (
                  <div className="glass-card glass-card-pad" style={{ padding: 24 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 12, background: "rgba(255,104,44,0.12)", display: "grid", placeItems: "center", flexShrink: 0 }}>
            ...
            {days.length ? (
              <section style={{ display: "grid", gap: 18 }}>
                {days.map((day: any, index: number) => (
                  <article key={index} className="glass-card day-card glass-card-pad" style={{ padding: 24, transition: "all 0.2s ease" }}>
                    <div className="day-card-header" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16, flexWrap: "wrap" }}>
                      <div style={{ width: 54, height: 54, borderRadius: 16, background: "linear-gradient(135deg, #0A1E3F 0%, #2654a3 100%)", color: "#fff", display: "grid", placeItems: "center", fontWeight: 800, fontSize: 18, flexShrink: 0 }}>{day.dayNumber ?? index + 1}</div>
                      <div style={{ minWidth: 0 }}>
                        ...
                        <h3 style={{ fontSize: "clamp(17px, 4vw, 20px)", fontWeight: 700, color: C.navy, margin: 0 }}>{day.title || `Day ${index + 1}`}</h3>
                      </div>
                    </div>
              ...
              <section className="glass-card glass-card-pad" style={{ padding: 24 }}>
          ...
          <aside style={{ display: "flex", flexDirection: "column", gap: 20, minWidth: 0 }}>
            <section className="glass-card glass-card-pad" style={{ padding: 24 }}>
              ...
              <div style={{ display: "grid", gap: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", color: C.textMuted, gap: 10 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, color: C.navy, gap: 10 }}>
                  <span>Total</span>
