...
// ============================================================
//  ResultsPage.tsx — composed from DateStrip, FilterPanel,
//  FlightCard and FareModal components.
//
//  All business logic below (state, effects, fetchFlights,
//  filtering, sorting, booking, combined-fare auto-pairing,
//  etc.) is unchanged. Only the render/JSX layer is organised
//  into smaller components and restyled to match the reference
//  UI — no prop, handler, or data-flow was altered.
// ============================================================

// import { useState, useMemo, useEffect, useCallback, useRef } from "react";
// import type { SearchForm, DisplayFlight, FareTier } from "../../lib/types_t";
// import { apiSearchFlights } from "../../lib/flights_api";
// import type { FlightSearchResult } from "../../lib/flights_api";
// import OneSearchBar from "./OneSearchBar";
// import SearchBar from "./organisms/SearchBar"
// import { useCurrency } from "../../context/currencyContext";

// import { S, type CityLeg, type ExtendedFilters, timeToMins, slotMatch, durationStr } from "./ResultShared";
// import DateStrip from "./DateStrip";
// import { FilterPanel, MobileFilterDrawer, SortChip } from "./FilterPanel";
// import { FlightCard, SkeletonCard, EmptyState, type FlightCardTag } from "./FlightCard";
// import { FareModal, findMatchingReturnLeg } from "./FareModal";
...
//   /** Cheapest / fastest tag for a card, computed against whichever
//    *  list it's actually rendered in (outbound vs. return share the
//    *  same helper but never each other's price/duration floor). */
//   const tagFor = (f: DisplayFlight, list: DisplayFlight[]): FlightCardTag | undefined => {
//     if (!list.length) return undefined;
//     const minPrice = Math.min(...list.map(x => x.price));
//     const minDur = Math.min(...list.map(x => x.duration));
//     if (f.price === minPrice) return "cheapest";
//     if (f.duration === minDur) return "fastest";
//     return undefined;
//   };
...
//         .date-strip-scroller::-webkit-scrollbar { display: none; }

//         @media (max-width: 1023px) {
//           .results-header-pad { padding: 12px 16px; }
//           .results-body { padding: 10px 12px; gap: 12px !important; }
//           .date-strip-badges { display: none !important; }
//         }
//         @media (max-width: 480px) {
//           .results-header-pad { padding: 10px 10px; }
//         }
...
//           {/* ── DATE STRIP ── (date/price scroller + Best Price Calendar / Price Alert / Lower Carbon badges) */}
//           {!isMultiCity && (
//             <DateStrip
...
//         }}
//       >
//         <div style={{ maxWidth: 1440, margin: "0 auto", padding: "12px 24px", display: "flex", alignItems: "stretch", width: "100%", overflowX: "auto", gap: 10 }}>
...
//                   fontFamily: "'Sora',sans-serif",
//                   color: active ? S.navyMid : S.navyDeep,
...
//                 {idx === 0 ? label : label}
//                 {subVal && (
//                   <span style={{ fontSize: 12, fontWeight: 800, color: active ? S.navyDeep : S.muted, textTransform: "none", letterSpacing: 0 }}>
//                     {subVal}
//                   </span>
//                 )}
//               </button>
...
//                 <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
//                   {filtered.map(f => (
//                     <FlightCard key={f.resultIndex} flight={f} onViewFares={setSelectedFlight} tag={tagFor(f, filtered)} />
//                   ))}
//                 </div>
//               )}
...
//                   <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
//                     {filteredReturn.map(f => (
//                       <FlightCard key={f.resultIndex} flight={f} onViewFares={setSelectedFlight} tag={tagFor(f, filteredReturn)} />
//                     ))}
//                   </div>
//                 </>
//               )}
...
// ============================================================
//  ResultsPage.tsx — assembles everything built across this
//  session into one working page: real search state, a live
//  flights fetch, client-side filtering feeding both the filter
//  panel and the flight list, and the date slider/search
//  summary kept in sync with the current search.
//
//  Layout/background is the same as the original page; the
//  only structural addition is making the filter sidebar
//  responsive (full-width block on mobile, sticky sidebar at
//  lg+) to match the components it now renders, which are
//  already mobile-aware.
// ============================================================

//  ResultsPage.tsx — assembles everything built across this
//  session into one working page.
//
//  Fixed from the previous version: apiSearchFlights actually
//  returns { outbound, returnFlights?, multiLegFlights?,
//  noResultReason? } — not a flat DisplayFlight[] — so
//  round-trip needs a real two-step flow: pick an outbound
//  flight first, then pick the return flight, THEN hand off to
//  booking. Multi-city (multiLegFlights, one array per leg) is
//  handled the same way in principle, but since the Multi-city
//  option is currently disabled in SearchBar, only the
//  one-way/round-trip paths are exercised in practice — the
//  multi-city branch below is a straightforward extension of
//  the same pattern once that UI is re-enabled.
// ============================================================
...
interface SearchResultsProps {
  /** The search the person actually submitted on SearchPage (or a prior
   *  search from this same page). This is what seeds the initial results —
   *  no more hardcoded CCU→DEL default. */
  form: SearchForm;
  multiLegs?: CityLeg[];
  /** Called once a fare is fully booked — one-way flights fire this once;
   *  round trips fire it twice (legIndex 0 = outbound, 1 = return), same
   *  contract the original ResultsPage used before this file was rewritten. */
  onBook?: (flight: DisplayFlight, tier: FareTier, legIndex?: number) => void;
  /** Optional "back to search" hook for a parent-owned page switcher. */
  onBack?: () => void;
  /** Called whenever this page runs a NEW search (SearchBar submit or
   *  DateSlider date change) — keeps the parent's own state/sessionStorage/
   *  URL in sync with whatever's actually on screen, instead of only living
   *  in this component's local state. */
  onNewSearch?: (form: SearchForm, multiLegs?: CityLeg[]) => void;
  /** Restores round-trip progress across a refresh/remount — the outbound
   *  flight/tier FlightsFlow already committed to state, if any. */
  selectedOutboundFlight?: DisplayFlight | null;
  selectedOutboundTier?: FareTier | null;
  /** Multi-city equivalent — not used by this component yet (multi-city
   *  search is disabled in SearchBar for now), accepted so the prop type
   *  matches what FlightsFlow actually passes. */
  selectedLegs?: Array<{ flight: DisplayFlight; tier: FareTier } | null>;
}
...
export default function SearchResults({
  ...
  // Raw search results, kept separate from the "which list is on screen
  // right now" question below.
  ...
  // Round-trip is a two-step pick: outbound first, then return. `null`
  // means "still choosing the outbound flight."
  ...
  const runSearch = useCallback((searchForm: SearchForm, legs?: CityLeg[]) => {
    ...
    // Tell the parent about every search that runs here — the initial one
    // seeded from `formProp`, a fresh SearchBar submission, or a DateSlider
    // date change — so its own state/sessionStorage/URL stay accurate.
  ...
  // Run the initial search using whatever was actually submitted on
  // SearchPage (passed down as `form`/`multiLegs`), instead of a hardcoded
  // default. Since FlightsFlow remounts this component (via its `key` prop)
  // whenever the search changes, this effect only needs to fire once per
  // mount — it's always seeded with the current search already.
  useEffect(() => {
    runSearch(formProp, multiLegsProp);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Which raw list is currently on screen — outbound until an outbound is
  // picked on a round-trip search, then the return list.
  const displayedFlights = isPickingReturn ? (returnFlights as DisplayFlight[]) : outboundFlights;

  // Client-side filtering — feeds both FilterPanela (so its header count/
  // airline list reflect what's actually shown) and FlightList.
  ...
  function handleBookFare(flight: DisplayFlight, tier: FareTier) {
    // Round-trip, outbound just picked, and there IS a return list to pick
    // from next → advance to the return-selection step locally (for the
    // list swap / banner), AND tell the parent right away. FlightsFlow's
    // handleBook keys off `!prevState.selectedFlight` to know this is leg 0
    // — it needs this call now, not bundled with the return leg's call
    // later, otherwise the outbound pick never reaches sessionStorage and
    // is lost on refresh.
    ...
    // Round trip, return leg just picked — a separate call, made later by
    // the user, after the parent has already committed leg 0 to state.
    if (form?.tripType === 'roundTrip' && selectedOutbound) {
      onBook?.(flight, tier, 1);
      return;
    }

    // One-way flight — single leg, no legIndex needed.
    onBook?.(flight, tier);
  }
  ...
  return (
    <div className="min-h-screen font-sans text-slate-900" style={{ background: `radial-gradient(circle at 50% -20%, rgba(82,145,255,.15), transparent 38%), radial-gradient(circle at 0% 20%, rgba(255,132,132,.06), transparent 30%), radial-gradient(circle at 100% 30%, rgba(88,170,255,.08), transparent 35%), linear-gradient(180deg, #FAFCFF 0%, #F6F9FD 40%, #EEF3FA 100%)` }}>

      {/* White hero zone behind search bar + date slider */}
      <div ref={heroRef} className="w-full bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 pb-4" style={{ paddingTop: 0, marginTop: 0 }}>
      ...
      <main className="max-w-7xl mx-auto px-4 pb-20 pt-6">
        ...
        {/* Column on mobile/tablet (filter panel above the list), original
            side-by-side row from lg up. */}
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          {/* Left filter — white card. `mobile` is passed here (even on
              desktop) because THIS wrapper already provides the sticky
              positioning/height — FilterPanela's own internal sticky
              sizing would otherwise double up. */}
          <div
            className="w-full lg:w-64 lg:shrink-0 rounded-xl p-4 flex flex-col lg:sticky lg:top-[72px]"
            style={{ background: 'rgba(255,255,255,0.28)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 8px 32px rgba(40,60,120,0.10), inset 0 1px 0 rgba(255,255,255,0.6)' }}
          >
