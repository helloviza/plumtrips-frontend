...
// src/pages/marketing/Holidays.tsx
...
interface ModalProps {
  mode: FormMode;
  initial: Omit<Holiday, "id"> & { id?: string };
  onClose: () => void;
  // Now accepts imageFile just like CruisesModal — consistent with shared API
  onSave: (data: Omit<Holiday, "id">, imageFile: File | null, id?: string) => Promise<void>;
}

function HolidayModal({ mode, initial, onClose, onSave }: ModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const { id, ...payload } = form as Holiday;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={form.scope}
                onChange={(e) => set("scope", e.target.value as HolidayScope)}
          ...
          {/* File upload — now consistent with Cruises.tsx */}
          <div>
        ...
        <div className="flex justify-end gap-3 px-6 pb-6">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 transition">Cancel</button>
          <button
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
        ...
        <h3 className="text-base font-bold text-slate-800 mb-1">Delete Holiday?</h3>
...
export default function HolidaysPage() {
  ...
  const [modal, setModal] = useState<{ mode: FormMode; item?: Holiday } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Holiday | null>(null);
  const [searchQ, setSearchQ] = useState("");
  const [scopeFilter, setScopeFilter] = useState<"All" | HolidayScope>("All");
  ...
  const handleSave = async (payload: Omit<Holiday, "id">, imageFile: File | null, id?: string) => {
    if (id) {
      const updated = await updateHoliday(id, payload, imageFile ?? undefined);
      setItems((p) => p.map((h) => (h.id === id ? updated : h)));
    } else {
      const created = await createHoliday(payload, imageFile!);
  ...
  const handleDelete = async () => {
    ...
    setItems((p) => p.filter((h) => h.id !== deleteTarget.id));
  ...
  const filtered = items.filter((h) => {
    const matchQ =
      ...
      h.subtitle.toLowerCase().includes(searchQ.toLowerCase());
  ...
  return (
    <div className="min-h-screen px-6 py-8" style={{ background: "#f0f4f8", fontFamily: "'Exo 2', system-ui, sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight" style={{ color: "#00477f" }}>Holidays</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-48 text-slate-400 text-sm">Loading holidays…</div>
        ) : filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((h, i) => (
                  <tr key={h.id} className={`border-b border-slate-50 hover:bg-slate-50/70 transition-colors ${i === filtered.length - 1 ? "border-b-0" : ""}`}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        {h.image && <img src={h.image} alt={h.title} className="w-10 h-10 rounded-lg object-cover shrink-0" />}
                        <div>
                    ...
                    <td className="px-5 py-4"><span className="font-semibold text-slate-700">{formatCurrency(h.price.toLocaleString("en-IN"))}</span></td>
                    <td className="px-5 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${h.scope === "International" ? "bg-blue-50 text-blue-600" : "bg-violet-50 text-violet-600"}`}>
                        {h.scope}
                      </span>
                    </td>
                    <td className="px-5 py-4"><StatusBadge active={h.active} /></td>
                    <td className="px-5 py-4">
                      <TrendingBadge trending={h.trending} />
                      {!h.trending && <span className="text-slate-300 text-xs">—</span>}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setModal({ mode: "edit", item: h })} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-[#00477f] bg-[#00477f]/8 hover:bg-[#00477f]/15 transition">Edit</button>
                        <button onClick={() => setDeleteTarget(h)} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-500 bg-red-50 hover:bg-red-100 transition">Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="mt-4 flex gap-4 text-xs text-slate-400">
        <span>{items.filter((h) => h.active).length} active</span>
        <span>·</span>
        <span>{items.filter((h) => !h.active).length} draft</span>
        <span>·</span>
        <span>{items.filter((h) => h.trending).length} trending</span>
      </div>
      ...
      {modal && (
        <HolidayModal
          mode={modal.mode}
          initial={modal.item ? { ...modal.item } : (blank() as Holiday)}
