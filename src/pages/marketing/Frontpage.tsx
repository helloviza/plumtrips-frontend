...
import { getFrontpage, updateFrontpage, deleteFrontpage, createFrontpage, type Frontpage, type FrontpageScope } from "../../lib/api";

type FormMode = "add" | "edit";

const blank = (): Omit<Frontpage, "id"> => ({
  title: "",
  subtitle: "",
  tag_one: "",
  tag_two: "",
...
interface ModalProps {
  mode: FormMode;
  initial: Omit<Frontpage, "id"> & { id?: string };
  onClose: () => void;
  onSave: (data: Omit<Frontpage, "id">, imageFile: File | null, id?: string) => Promise<void>;
}

function FrontpageModal({ mode, initial, onClose, onSave }: ModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const { id, ...payload } = form as Frontpage;
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Tag One</label>
              <input
                value={form.tag_one}
                onChange={(e) => set("tag_one", e.target.value)}
                placeholder="e.g. Beach"
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-[#00477f]/30 focus:border-[#00477f] transition"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Tag Two</label>
              <input
                value={form.tag_two}
                onChange={(e) => set("tag_two", e.target.value)}
          ...
          {/* Extra Info */}
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Extra Info</label>
            <textarea
              value={form.extra_info}
              onChange={(e) => set("extra_info", e.target.value)}
              placeholder="Any additional details…"
              rows={3}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-[#00477f]/30 focus:border-[#00477f] transition resize-none"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={form.scope}
              onChange={(e) => set("scope", e.target.value as FrontpageScope)}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
        ...
        <h3 className="text-base font-bold text-slate-800 mb-1">Delete Section?</h3>
...
export default function FrontpagePage() {
  const [items, setItems] = useState<Frontpage[]>([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal] = useState<{ mode: FormMode; item?: Frontpage } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Frontpage | null>(null);
  const [searchQ, setSearchQ] = useState("");
  const [scopeFilter, setScopeFilter] = useState<"All" | FrontpageScope>("All");
  ...
  const handleSave = async (payload: Omit<Frontpage, "id">, imageFile: File | null, id?: string) => {
    if (id) {
      const updated = await updateFrontpage(id, payload, imageFile ?? undefined);
      setItems((p) => p.map((c) => (c.id === id ? updated : c)));
    } else {
      const created = await createFrontpage(payload, imageFile!);
  ...
  const handleDelete = async () => {
    ...
    await deleteFrontpage(deleteTarget.id);
  ...
  return (
    <div className="min-h-screen px-6 py-8" style={{ background: "#f0f4f8", fontFamily: "'Exo 2', system-ui, sans-serif" }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight" style={{ color: "#00477f" }}>Frontpage Sections</h1>
          <p className="text-sm text-slate-500 mt-0.5">{items.length} section{items.length !== 1 ? "s" : ""} total</p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {(["All", "HOLIDAYS", "CRUISES", "FLIGHTS", "HOTELS", "OFFERS", "VISAS", "BLOGS", "STOPOVER"] as const).map((s) => (
            <button
              key={s}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-48 text-slate-400 text-sm">Loading sections…</div>
        ) : filtered.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  {/* ✅ Fixed: removed "Price", columns now match cells exactly */}
                  {["Destination", "Scope", "Status", "Trending", "Actions"].map((h) => (
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {/* ✅ Show tags inline under the title */}
                          <div className="flex gap-1 mt-0.5 flex-wrap">
                            {c.tag_one && (
                              <span className="text-[10px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded-full">{c.tag_one}</span>
                            )}
                            {c.tag_two && (
                              <span className="text-[10px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded-full">{c.tag_two}</span>
                            )}
                          </div>
                        </div>
                      </div>
                    </td>
                    {/* ✅ Scope — was incorrectly in the "Price" column */}
                    <td className="px-5 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${
                        c.scope === "HOLIDAYS" ? "bg-blue-50 text-blue-600"
                        : c.scope === "CRUISES" ? "bg-violet-50 text-violet-600"
                        : c.scope === "OFFERS" ? "bg-amber-50 text-amber-600"
                        : c.scope === "FLIGHTS" ? "bg-sky-50 text-sky-600"
                        : c.scope === "HOTELS" ? "bg-emerald-50 text-emerald-600"
                        : c.scope === "VISAS" ? "bg-rose-50 text-rose-600"
                        : c.scope === "BLOGS" ? "bg-indigo-50 text-indigo-600"
                        : "bg-slate-100 text-slate-600"
                      }`}>
                        {c.scope}
                      </span>
                    </td>
                    {/* ✅ Status — was incorrectly in the "Scope" column */}
                    <td className="px-5 py-4"><StatusBadge active={c.active} /></td>
                    {/* ✅ Trending — was incorrectly in the "Status" column */}
                    <td className="px-5 py-4">
                      <TrendingBadge trending={c.trending} />
                      {!c.trending && <span className="text-slate-300 text-xs">—</span>}
                    </td>
                    {/* ✅ Actions — was incorrectly in the "Trending" column with an empty last cell */}
                    <td className="px-5 py-4">
      ...
      {modal && (
        <FrontpageModal
          mode={modal.mode}
          initial={modal.item ? { ...modal.item } : (blank() as Frontpage)}
