...
function normHotel(
  ...
  const price = cheapest ? Number(cheapest.TotalFare ?? 0) : 0;
...
function normRoom(raw: any, index: number): Room {
  ...
  const price = Number(raw.TotalFare ?? 0);
...
export function useHotelSearch() {
  ...
  const search = useCallback(async (params: HotelSearchInput) => {
    ...
    try {
      // Step 1: Get full hotel list for the city (includes name, address, rating)
      setStatusMessage('Finding hotels in this city…');
      const cityHotels: any[] = await getCityHotels(params.cityCode);
      ...
      // Build a lookup map: HotelCode → city hotel metadata
      ...
      // Take first 100 codes for the search call
      ...
      // Step 2: Search for available rooms (server returns traceId for prebook/book chain)
      ...
      // Step 3: Optionally enrich with static details (images, amenities)
      ...
      try {
        ...
        // Static details are optional — continue without them
      }

      // Merge: for each search result, find its city hotel metadata + static details
      const normalized = searchResults.map((sr: any) => {
        ...
        return normHotel(cityHotel, sr, sd);
      });
  ...
  return { hotels, rawResults, loading, error, statusMessage, search };
}
