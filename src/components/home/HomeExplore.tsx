...
/**
 * HomeExplore (brand-styled + mobile & desktop friendly, ~15% smaller scale)
 */
...
export default function HomeExplore() {
  ...
  return (
    <section className="relative mx-auto max-w-6xl px-4 pb-12 pt-6 sm:pt-8">
      {/* Section header with accent bar */}
      <div className="mb-5 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <span
            className={`inline-block h-5 w-1.5 rounded-full ${BRAND.primaryBg}`}
            aria-hidden
          />
        ...
        <div className="md:hidden">
          <Link
            to="/offers"
            className={`inline-flex w-full items-center justify-center gap-2 rounded-lg border px-3 py-1.5 text-[11px] font-semibold ${BRAND.primaryText} border-zinc-200 bg-white/70 backdrop-blur hover:bg-zinc-50`}
          >
            View latest offers
            <ArrowRight />
          </Link>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
        ...
        {/* Sidebar */}
        <aside className="space-y-3.5 lg:col-span-3">
          <SidebarCard
...
function SidebarCard({
  ...
  const Action = onActionClick ? (
    <button
      ...
      className="mt-2.5 inline-flex items-center justify-center rounded-md border border-zinc-300 px-2.5 py-1.5 text-[11px] font-semibold text-zinc-800 hover:bg-zinc-50 sm:mt-0"
    >
      {actionLabel}
    </button>
  ) : (
    <Link
      to={to}
      className={
        "mt-2.5 inline-flex items-center justify-center rounded-md px-2.5 py-1.5 text-[11px] font-semibold sm:mt-0 " +
        (variant === "accent"
  ...
  return (
    <div
      className={
        "flex flex-col items-start gap-2.5 rounded-2xl border p-3 shadow-sm transition hover:shadow-md sm:flex-row sm:items-center sm:justify-between " +
        (variant === "accent"
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`grid h-8 w-8 place-items-center rounded-lg ${BRAND.primaryBg} text-white shadow-sm`}
        >
          {icon || <SparklesIcon />}
        </div>
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <div className="text-[11px] font-semibold text-zinc-900 sm:text-xs">
          {heading}
        </div>
      </div>
        ...
        {items.map((t) => {
          ...
          return (
            <Link
              key={label}
              to={to}
              className="rounded-full border border-zinc-200 bg-white px-2 py-1 text-[10px] font-medium text-zinc-700 hover:border-sky-300 hover:text-sky-800 hover:shadow-sm sm:px-2.5 sm:text-[11px]"
              onClick={(e) => {
                e.preventDefault();
                navigate(to);
              }}
